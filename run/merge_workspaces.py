...
if __name__ == "__main__":
    ...
    for ifile in xrange(0, len(args.file_in)):
        label = GetLabel(ifile)
        file_in = ROOT.TFile(args.file_in[ifile], "read")
        wspace_in = file_in.Get(args.wspace_in)

        getattr(wspace_out, "import")(wspace_in.allPdfs(),
                                      ROOT.RooFit.RenameAllVariablesExcept(label, ",".join(args.shared)),
                                      ROOT.RooFit.RenameAllNodes(label))
        for var_set in var_sets:
            ExtendSet(wspace_out, wspace_in, var_set, label, args.shared)

        file_in.Close()

    AddLikelihood(wspace_out, "model_b", len(args.file_in))
    AddLikelihood(wspace_out, "model_s", len(args.file_in))
